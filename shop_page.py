...
# Add this at the very top of shop_page.py (adjust 'database' if your file name is different)
from database import Product
from database import Order, Product, TryOnFeatureMeter

# Assuming your database SQLAlchemy models are imported directly from your database module
...
def render_marketplace_hub(db, token_user_id, COMMISSION_RATE=0.10):
    ...
    with st.expander(
        ...
        if not active_cart_orders:
            ...
            # =========================================================================
            # 💳 REWRITE: SWITCH CHANNELS TO NATIVE PAYHUB PAYMENTS ORCHESTRATOR 💳
            # =========================================================================
            if st.button(
                "💳 Proceed to Checkout (PayHub Gateway Integration)",
                use_container_width=True,
            ):
                with st.spinner("Connecting securely to PayHub API gateway..."):
                    time.sleep(1.0)  # Simulate network tokenization latency

                    # Update order statuses inside your Supabase transactional loop
                    for active_item in active_cart_orders:
                        ...
                        # Generate unique dynamic transaction tracker references matching PayHub blueprints
                        active_item.stripe_session_id = (
                            f"ph_live_trx_{int(time.time())}_{active_item.id}"
                        )

                    db.commit()
                    st.success(
                        "🎉 Payment verified via PayHub Engine! Funds routed securely across merchant balances."
                    )
                    time.sleep(0.8)
                    st.rerun()
    ...
    for row_idx in range(0, len(catalog_products), 3):
        ...
        for i, prod in enumerate(catalog_products[row_idx : row_idx + 3]):
            with cols[i]:
                    ...
                    with st.spinner("Processing optimization textures..."):
                        try:
                            ...
                            if p_bytes is None:
                                ...
                                # Validate garment category values safely to match engine options
                                category_value = str(
                                ...
                                # Render output lookbook matrix element card onto the grid block area
                                st.image(
                                    lightweight_jpeg_bytes,
                ...
                # If limits have been crossed, render the PayHub Premium expansion gateway button cell
                if disabled_flag:
                    ...
                    if st.button(
                        ...
                        db.commit()
                        st.success(
                            "PayHub Token Received: Premium Unlimited Access enabled!"
                        )
                        time.sleep(0.5)
                        st.rerun()
